...
pipe = joblib.load('../Model-scikit/model.pkl')

# Mediapipe setup
mp_pose = mp.solutions.pose
mp_drawing = mp.solutions.drawing_utils  # Not used in this version, but kept for potential future use
pose = mp_pose.Pose()
...
for stage in stage_times:
    print(f"{stage}:")
    print(f"  - Time Percentage: {percentages[stage]:.2f}%")
    print(f"  - Frame Count Percentage: {frame_counts[stage]:.2f}%")
...
elif percentages['Low Back'] > 30:
    print("\nFocus on keeping your back straight and avoid sagging in the middle.")
elif percentages['High Back'] > 30:
    print("\nTry to engage your core more and avoid raising your hips too high.")

cap.release()
# cv2.destroyAllWindows()  # No need to destroy windows since we're not creating any